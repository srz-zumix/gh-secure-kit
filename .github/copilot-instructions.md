...
* ディレクトリ・ファイル構成は以下の責務分割に従うこと
  * cmd/: CLIコマンド定義・引数/フラグ処理・cobra.Command生成のみを担当し、ビジネスロジックは持たない
    * cmd/直下に主要コマンド（actions.go, deps.go, root.go など）を配置
    * cmd/actions/ に actions サブコマンドのサブコマンドを配置
    * cmd/deps/ に deps サブコマンドのサブコマンドを配置
    * cmd/deps/actions/, cmd/deps/submodule/, cmd/deps/unity/ などの細分化されたコマンドを配置する場合がある
...
* サブディレクトリ構成例:
  * cmd/actions/lint.go, cmd/actions/workflow.go など
  * cmd/deps/actions/list.go, cmd/deps/actions/graph.go などの多階層構成も可
